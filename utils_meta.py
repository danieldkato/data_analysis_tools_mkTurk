...
def get_recording_path(base_data_path, monkey, date,depth = 4):
    ...
    long_paths = os_sorted(((base_data_path/monkey).glob('*' + date + '*')))
            ...
            for b_f in behav_file_list:
                ...
                if m['TASK']['RewardStage'] == 1:
                    for samplescene in m['SCENES']['SampleScenes']:
                        stims.extend(np.unique(list(map(gen_short_scene_info,gen_scene_df(samplescene)))))

    return np.unique(np.array(stims))

def get_allsess_stim(base_data_path,monkey,stim_list, base_save_out_path):
        ...
        for dp in data_path:
            ...
            try: # try look for stim_sess_info since it already has all stimulus id used in the session 
                ...
                if len(set(stim_keys_sess).intersection(stim_list)) > 0:
                    allsess.append(f)
                    print(f)
            except: # if stim_sess_info is not there, 
...
def get_allsess_scenefile(base_data_path,monkey,scenefile_list, base_save_out_path):
    folders = os_sorted([f for f in next(os.walk(base_data_path/monkey))[1] if monkey in f])
        ...
        if len(behav_file_list) == 0:
            ...
            for b_f in behav_file_list:
                ...
                if m['TASK']['RewardStage'] == 1:
                    ...
                    scenefile = 'calibration'
                df_behavior.loc[len(df_behavior.index)] =[monkey, date, b_f.stem,scenefile ]
...
def init_dirs(base_data_path, monkey, date, base_save_out_path):
    ...
    print('data_path_list = {}'.format(data_path_list))
